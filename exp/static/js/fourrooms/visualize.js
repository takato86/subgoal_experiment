...
function renderSubgoals(header, data){
    init();
    const i_state = header.indexOf('state');
    let state;
    let subg_counter = {}
    for(row of data){
        state = parseInt(row[i_state]);
        if(subg_counter[state]){
            subg_counter[state] += 1;
        }else{
            subg_counter[state] = 1;
        }
    }
    render();
    render_subgoals(subg_counter);
}

function render_subgoals(subg_counter){
